...
((Drupal,once)=>{Drupal.behaviors.commerceInbox={attach:(context)=>{once('commerceInboxMessageToggle','.inbox-message',context).forEach((message)=>{message.addEventListener('click',()=>{if(message.classList.contains('opened'))return;message.classList.add('opened');if(message.classList.contains('unread'))Drupal.ajax({url:Drupal.url(`admin/commerce/inbox-message/${message.dataset.messageId}/read`)}).execute();});if(message.querySelector('.close'))message.querySelector('.close').addEventListener('click',(e)=>{e.stopPropagation();e.target.closest('.inbox-message').classList.remove('opened');});});}};})(Drupal,once);;
((Drupal,once)=>{Drupal.behaviors.managementLinks={attach(context){once('managementLinksToggle','.management-links-toggle',context).forEach((toggle)=>{toggle.addEventListener('click',this.toggleLinks);});},toggleLinks:(event)=>{const toggle=event.currentTarget;toggle.classList.toggle('active');const target=toggle.getAttribute('aria-controls');const targetElements=document.getElementsByClassName(target);[...targetElements].forEach((element)=>{element.classList.toggle('show');});}};})(Drupal,once);;
